...
export function aggregateData(labels: Ref, items: any[], groupBy: 'day' | 'month' | 'year', callback: (item: any) => number): number[] {
    const aggregatedData = [];
    for (let i = 0; i < labels.value.length; i++) {
        aggregatedData.push(0);
    }
    items.forEach((item: any) => {
        ...
        const itemIndex = labels.value.findIndex((label: string) => label === itemFilter);
        aggregatedData[itemIndex] += callback(item);
    });
    return aggregatedData;
}
