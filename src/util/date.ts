...
export function getTicketNumberByMonthAndYear(
  monthYear: string,
  ticketInfos: MonthlyTicketInfos[],
  filter: string
): number {
  const monthYearToCompare = monthYear.split("/").reverse().join("-");
  const ticketInfo = ticketInfos.find(
    (info) => monthYearToCompare === info.monthYear.toString()
  );
  switch (filter) {
    case "nbTotal":
...
export function getTicketNumberByDay(
  dayMonthYear: string,
  ticketInfos: DailyTicketInfos[],
  filter: string
): number {
  const dayMonthYearToCompare = dayMonthYear.split("/").reverse().join("-");
  const ticketInfo = ticketInfos.find(
    (info) => dayMonthYearToCompare === info.dayMonthYear.toString()
  );
  switch (filter) {
    case "nbTotal":
      return ticketInfo ? ticketInfo.numberStatsTicket.nbTotal : 0;
